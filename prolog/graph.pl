% Originally from https://github.com/damianoazzolini/graphpl/blob/master/prolog/graph.pl
...
% find_path_unweighted(+Graph,+V1,+V2,-Path)
% find all paths between V1 and V2 in a directed weighted graph and ignore Costs
find_path_weighted(graph(Vertices,Edges),V1,V2,Path):-
    member(V1,Vertices),
    member(V2,Vertices),
    find_path_weighted_(V1,V2,Edges,[V1],Path).

find_path_weighted_(V,V,_,L,L).
find_path_weighted_(V1,V2,Edges,L,Path):-
    % connected(V1,V2,Edges),
    member(edge(V1,V2,_),Edges),    
    append(L,[V2],Path).
find_path_weighted_(V1,V2,Edges,L,Path):-
    % connected(V1,VX,Edges),
    member(edge(V1,VX,_),Edges),
    VX \== V2,
    \+member(VX,L),
    append(L,[VX],L1),
...
delete_vertex_unweighted(V, graph(VIn, EIn), graph(VOut, EOut)) :-
    member(V, VIn), delete(VIn, V, V0), sort(V0, VOut),
    findall(X, (member(X, EIn), X = edge(_, V)), EXV),
...
copy_vertex_unweighted(VBase, VCopy, graph(VIn, EIn), graph(VOut, EOut)) :-
    member(VBase, VIn), union(VIn, [VCopy], V0), sort(V0, VOut),
    findall(Z, (member(X, EIn), X = edge(Y, VBase), Z = edge(Y, VCopy)), EXV),
    findall(Z, (member(X, EIn), X = edge(VBase, Y), Z = edge(VCopy, Y)), EVX),
    union(EIn, EXV, E0), union(E0, EVX, E1), sort(E1, EOut).    


% Copies some vertex from graph(+VIn, +EIn)
copy_vertex_weighted(VBase, VCopy, graph(VIn, EIn), graph(VOut, EOut)) :-
    member(VBase, VIn), union(VIn, [VCopy], V0), sort(V0, VOut),
    findall(Z, (member(X, EIn), X = edge(Y0, VBase, Y1), Z = edge(Y0, VCopy, Y1)), EXV),
    findall(Z, (member(X, EIn), X = edge(VBase, Y0, Y1), Z = edge(VCopy, Y0, Y1)), EVX),
    union(EIn, EXV, E0), union(E0, EVX, E1), sort(E1, EOut).    

% Replaces some vertex from graph(+VIn, +EIn)
replace_vertex_unweighted(VOrigin, VReplace, graph(VIn, EIn), graph(VOut, EOut)) :-
    copy_vertex_unweighted(VOrigin, VReplace, graph(VIn, EIn), graph(VCpy, ECpy)),
    delete_vertex_unweighted(VOrigin, graph(VCpy, ECpy), graph(VOut, EOut)).

% Replaces some vertex from graph(+VIn, +EIn)
replace_vertex_weighted(VOrigin, VReplace, graph(VIn, EIn), graph(VOut, EOut)) :-
    copy_vertex_weighted(VOrigin, VReplace, graph(VIn, EIn), graph(VCpy, ECpy)),
    delete_vertex_weighted(VOrigin, graph(VCpy, ECpy), graph(VOut, EOut)).